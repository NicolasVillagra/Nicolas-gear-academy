...
use gstd::{
    prelude::*,msg,ActorId,collections::{BTreeSet, BTreeMap},exec,ReservationId
};
...
impl Battle {
    pub async fn initiate_registration(&mut self, tmg_id: &TamagotchiId, attributes: AttributesPerRound) {
        assert_eq!(
            self.state,
